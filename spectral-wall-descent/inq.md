...
[[vendor/postquantum-gravity/inq|Postquantum gravity]] begins from a fundamental classical--quantum split and asks which completely positive stochastic dynamics can preserve it. Its decoherence--diffusion trade-off and Onsager--Machlup history weight are therefore downstream of the question posed here. At trade-off saturation, purity conditioned on a classical trajectory is an exact result in that theory; it does not imply unitarity of this wall, because the classical trajectory and the action over histories have already been assumed.
