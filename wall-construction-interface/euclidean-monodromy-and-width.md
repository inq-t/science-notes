...
The categorical obstruction is that angular periodicity naturally constrains a vertical rotation generator, whereas \(\varrho_\perp\) is a horizontal comparison slope. No equality between those generators is presently known.

## Upgrade criterion

A successful construction must:

1. construct the Euclidean wall and its state bundle;
2. show that horizontal transport extends consistently around the angular circle;
3. identify the resulting holonomy or monodromy representation;
4. map its normalized generator to the binary \(Q^2=\mathbf1\) comparison channel; and
5. derive the allowed value of \(\varrho_\perp\) without rescaling \(N\), \(Q\), or \(\theta\) afterward.
