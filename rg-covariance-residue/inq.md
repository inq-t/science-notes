...
[[gaussian-readout-naturality|Gaussian readout naturality]] gives a separate clock-selection theorem: a reversible constant-mobility diffusion depending only on the covariance and compatible with every surjective linear readout must use covariance-proportional mobility, with one common rate. Its harmonic split assigns that same rate to retained and residual directions. This constructs an auxiliary Gaussian response; it does not change the preceding Maxwell conclusion in the physical metric.
