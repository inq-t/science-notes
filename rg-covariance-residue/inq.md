...
[[endpoint-averages-and-quadratic-ultraviolet-control|Endpoint averaging]] proves a full observation inequality: curvature and the retained average jointly control every linear gauge class. [[soft-gaussian-gauge-blocking|Soft Gaussian blocking]] uses it to bound reverse conditional precision uniformly in volume and completed depth. Its noise sum is geometric, so no per-step loss accumulates. [[local-completion-of-soft-gauge-conditioning|A local partial-divergence completion]] separately proves spatial covariance and source-response decay for the first Maxwell step, preserving the exact curvature law. Locality of the accumulated precision remains a further obligation. [[compact-gauge-kernel-tangent-response|The compact-kernel tangent calculation]] supplies the normalized near-identity match in four dimensions and separates a mode Hessian from the forward Fisher metric. The Gaussian precision bound is depth-uniform, but the compact approximation is only fixed-regulator; transferring the estimates through the full nonlinear law remains open.
