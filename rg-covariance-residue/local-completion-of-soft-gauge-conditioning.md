...
[[multilevel-local-gauge-completion|The full-chain enlargement]] supplies the actual terminal Schur operator. [[uniform-gaussian-conditional-locality|The separate accumulated-locality proof]] controls its precision coefficients in a common complex strip and inverts it in a weighted norm, bypassing the coherent-mode obstruction of the full stack. The nonlinear obligation still includes the full Wilson law, large fields, holonomies, and renormalized source norms. Finally the retained infrared term must be controlled: the unconditioned initial Maxwell law remains massless despite the localized conditional fluctuations.

[[receipts/endpoint_average_receipt.py|The finite receipt]] checks the adapted completion against the quotient covariance and mean response, the norm constants, gauge-parameter independence, and the naive-completion counterexample. Its multilevel tests additionally compare the actual Schur law and alias formula. These checks supplement the proofs; they do not certify nonlinear Yang--Mills conclusions.
