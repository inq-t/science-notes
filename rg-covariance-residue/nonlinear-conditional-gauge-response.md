...
The [[uniform-gaussian-conditional-locality|depth-uniform Gaussian result]] controls a different, linear near-identity regime. Neither result bridges the non-Abelian crossover automatically. Nor can the exceptional representation's positive identity metric do so: [[exceptional-gauge-realization/faithful-and-adjoint-holonomy-response|the faithful color potential]] has negative Hessian at nontrivial central holonomies. Positive configuration-space Ricci curvature can dominate that potential curvature in (NG9), but it is not spacetime curvature or a physical mass by itself.
