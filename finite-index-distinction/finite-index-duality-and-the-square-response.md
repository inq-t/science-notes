...
A chosen normal faithful finite-index expectation between properly infinite von Neumann algebras supplies a conjugate intertwiner whose squared norm is its central Kosaki index. Normalizing gives an isometry, proper in a factor when that index exceeds one. Minimal expectation and standard conjugate data further identify the square with intrinsic categorical dimension and make its logarithm fusion additive. These are exact algebraic normalizations; vacuum-selective physical response, its scale family and its energy comparison remain separate constructions.
