...
A finite-index expectation between infinite-dimensional factors has log index as its maximal relative-entropy loss and, under the stated standard-form hypotheses, as a budget shared with its commutant dual. Standard minimal sectors give a capacity \(2\log d(X)\) additive under fusion. A cosmic application must independently choose and prove its capacity weld, select a wall and supply an absolute scale. Maximum information loss alone does not imply the minimum physical response required by a Yang--Mills mass gap.
...
[[finite-index-distinction/receipts/two-sided-index-capacity-receipt.py|The finite receipt]] and its [[finite-index-distinction/receipts/two-sided-index-capacity-receipt-output.txt|stored output]] check the half-index/full-index distinction, exhibit ancillary-carrier dependence, and illustrate the declared scalar ladder and rate-quotient arithmetic. They do not prove either finite supremum, test Connes fusion or the Type-III theorem, construct the physical weld or wall selection, supply a common carrier, or establish any Yang--Mills estimate.
