...
The open construction must separate those effects and determine whether \(C_\perp(N)\) is constant in the physical comparison scheme. [[program-core/localized-areal-response-geometry|Localized areal response geometry]] states the stronger local requirement: construct a measure-valued BKM form and compare it with independently normalized area. [[deriving-value-of-g/obstructions-to-an-unconditional-proof|The replication and dimensional obstructions]] show why the normalized binary algebra cannot settle this question.
