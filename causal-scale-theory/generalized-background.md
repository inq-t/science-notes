...
The generalized CST background composes a reduced binary shape, a constitutive homogeneous source, present-flatness branch selection, and ordinary GR--FLRW evolution. It is a **[CONDITIONAL OUTPUT]** whose response has two independent parameters—the scale-state rate \(\nu\) and the integrated crossing ratio \(\mathfrak R_c\)—while abundances, root branch, and residual-sector data remain additional background inputs. Neither response parameter is a universal constant merely because the unit branch proposes the value one.
...
- \(\nu:=|\mathrm d\theta/\mathrm dN|>0\) is the constant scale-state rate, hence an inverse profile width, after affine scale--state soldering has been granted;
