...
with non-circular cross-fiber transport. [[wall-construction-interface/entry|The wall-construction interface]] owns this decisive construction, now resolved into separate gates rather than one omnibus gap. [[wall-construction-interface/core-spectral-wall|The core spectral wall]] realizes an exact algebraic pre-wall without \(G\) or a fitted expansion history: its finite corners carry trace-capacity, while labelled copies of the whole core carry one faithful state orbit, exact transport, a state-preserving binary readout, and the balanced Fisher--BKM pulse. The readout does not live on the finite corners.
