...
$$
\rho_i(x)=\bar\rho_i\bigl(N+\zeta_{\mathrm{clock}}(x)\bigr).
$$

This is **[ASSUMPTION — CWST MEMBER]** until a wall-to-matter coupling derives it. It is not a consequence of one reflection-odd wall generator, one spin-zero stress form factor, or a balanced binary channel. Those are three different uses of “rank one.”

Given the ansatz, [[compatible-with-existing-physics/passive-adiabatic-transfer#Common-clock lemma|the common-clock lemma]] gives \(S_{ij}=0\) at linear order. To reach conserved curvature one must additionally suppress intrinsic nonadiabatic pressure, control interactions and anisotropic stress, select the passive growing mode, and match through the transition into the hot universe.
