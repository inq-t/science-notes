...
by [[basic-concepts/hessians/symmetrized-relative-entropy-hessian|the symmetrized-Hessian theorem]]. This factor of two comes from adding the two orientations of relative entropy. It does not show that the reverse orientation is a complex-conjugate wavefunctional, that \(G^{\mathrm{BKM}}\) equals a stress discontinuity, or that the result is the exact inverse covariance of an interacting field. A physical transfer may contain a nontrivial frequency kernel rather than a scalar normalization.
