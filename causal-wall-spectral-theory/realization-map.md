...
&&\text{center-resolved scalar-sector response},\\
\text{W2}_{\mathrm s}\text{: }&
\mathcal K^{\zeta}_{\mathrm{wall}}
=\mathcal W^{\mathsf p_{\mathrm s}}_{\mathrm{BKM}\to\mathrm{spatial}}
...
&&\text{center-resolved transverse-traceless response},\\
\text{W2}_{\mathrm t}\text{: }&
\mathcal K^{\gamma}_{\mathrm{wall}}
=\mathcal W^{\mathrm{TT},\mathsf p_{\mathrm t}}_{\mathrm{BKM}\to\mathrm{spatial}}
...
Here \(S_{\mathsf p}\) is a declared normal unconditioned or normalized internal-sector policy owned by [[program-core/center-valued-response|center-valued response and scalarization]]. A center-linear W2 member may instead retain a center-valued precision through the later maps. Algebraic character evaluation is neither normalized conditioning nor an actual outcome without an instrument. The scalar and tensor policies need not coincide.
...
|---|---|---|
| W0, full canonical package | [[wall-construction-interface/entry|wall-construction interface]] | construct the regions, horizon or boundary, cut, local algebras, states, modular data, transport, state law, and renormalized source without solving backward from the desired spectrum |
...
| W2\(_{\mathrm s}\) | [[conjectures/state-response-is-spatial-precision|CWST scalar state-to-spatial-precision conjecture]] | declare the central-resolution policy and construct every required carrier, state-response, normalization, and—where used—continuation arrow in the scalar sector |
| W2\(_{\mathrm t}\) | [[conjectures/tensor-state-response-is-spatial-precision|CWST tensor state-to-spatial-precision conjecture]] | declare the independent TT central-resolution policy, carrier, response transform, polarization convention, and normalization |
| critical \(|k|^3\) shape | [[critical-scale-kernels/entry|critical scale kernels]] | justify its symmetry hypotheses for the returned field |
| optional stress representation | [[spectral-realization|the holographic spectral adapter]] and [[vendor/holographic-cosmology/entry|holographic cosmology]] | establish the wall-to-\(T^{(3)}\) source pairing, state, branch, regulator, simultaneous continuation, and contacts; the adapter cannot discharge W3 |
...
| W3\(_{\mathrm t}\) | [[conjectures/wall-tensor-is-physical-graviton|CWST tensor realization conjecture]] | construct the independent gauge-reduced Lorentzian TT phase space, state, and normalization |
...
The core's local areal response density and CWST's Fourier precision are not the same coefficient:
...
in the registered three-dimensional Fourier convention. A common-geometry proposal must therefore supply an integration over a normal or history direction, a boundary-to-bulk map, a measure conversion, or another explicit soldering operation. [[program-core/common-response-matrix|The core common response form]] owns the shared blocks, while [[conjectures/homogeneous-inhomogeneous-common-geometry|the CWST consumer note]] owns this stronger application. Dimensional agreement after an unstated unit choice would not be enough; the carriers and measures must also agree.
...
- Failure of W4 leaves scalar and tensor two-point members possible but prevents higher-point and non-Gaussianity claims.
- Failure of W5 rejects observable adequacy or the imported-transfer route.
- Failure of the strong common-geometry conjecture prevents unification with the homogeneous areal response, but need not refute a weaker independently postulated spectral interface.

The status of every downstream claim is bounded by the weakest edge it uses.
