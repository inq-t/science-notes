...
One optional representation uses [[vendor/holographic-cosmology/entry|holographic cosmology]]. In that vendored framework a continued three-dimensional stress response can encode scalar and tensor spectra. [[spectral-realization|The CWST spectral adapter]] keeps a response calculated from a microscopic member separate from a response inferred backward from observed powers. Holography is therefore a replaceable implementation of the spectral middle layer, not the definition or ontology of a causal wall.
