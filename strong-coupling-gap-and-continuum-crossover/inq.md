...
[[wilson-slab-conditional-fisher-certificate#An interacting finite gauge bridge through time refinement|The complete finite-gauge computation]] follows a different, diagnostic leg: adjacent plaquettes and a three-dimensional \(\mathbb Z_2\) cube retain their magnetic interaction while the temporal spacing tends to zero at fixed slab thickness. It computes both the complete bridge floor and the excitation rate, showing why the Gaussian equality cannot replace the general one-sided comparison. Its graph and finite gauge group stay fixed.

## The continuum trajectory leaves that theorem's regime

[[temporal-column-response/inq|Temporal-column response]] improves the time-refinement leg without dropping the interaction: a complete path is one variable in a compact Wasserstein metric, and its conditional block mixing bounds the cost of a local exterior insertion. [[wilson-temporal-column-coercivity|The Wilson application]] yields an explicit vacuum Poincare floor uniform in spatial volume and \(x\to\infty\) when \((d_s-1)\beta_s x\le1/200\). Its [[temporal-column-response/smoothed-endpoint-response|distributed endpoint score]] additionally gives a complete two-ended bridge floor \(\kappa_*\simeq0.1594012596\) at half-depth \(2\lceil4x\rceil\). [[wilson-to-hamiltonian-vacuum-limit|The vacuum-limit theorem]] and joint-law convergence pass the bounds to the continuous-time lattice Hamiltonian at fixed spatial regulator. The small magnetic/electric ratio still excludes the spatial continuum trajectory; no finite-step Wilson-to-Laplacian form domination is used.
