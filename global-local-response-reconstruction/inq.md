...
[[exceptional-context-analysis-of-gauge-gradients|Exceptional context analysis]] now supplies one explicit field-sensitive map: apply the context losses to differentiated gauge-representation gradients, then integrate along the whole law's auxiliary heat flow. This factors a bounded response on the retained physical carrier. It also explains why [[exceptional-state-comparison/primitive-peirce-response|complete finite matrix readouts]] alone are insufficient: a fiberwise unital map leaves every scalar function of the field untouched. Uniform comparison with the joint-boundary defect below remains a separate theorem.
