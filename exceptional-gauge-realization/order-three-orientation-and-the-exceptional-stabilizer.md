...
K:=C_{F_4}(w)
\cong
\frac{SU(3)\times SU(3)}{\Delta\mathbb Z_3},
\qquad
\Delta\mathbb Z_3
:=
...
Thus, relative to the oriented lift \(w\), \(I_w\) is an orthogonal complex
structure on the forgotten real space \(B^\perp\). It is not canonical from
\(B\) alone. If \(\kappa\) belongs to the antiunitary component of
\(\operatorname{Stab}(B)\), then

$$
\kappa w\kappa^{-1}=w^2,
\qquad
\kappa I_w\kappa^{-1}=I_{w^2}=-I_w,
\qquad
P_{\operatorname{Fix}(w^2)}
=P_{\operatorname{Fix}(w)}
=P_B.
\tag{OT8}
$$
...
is a literal two-to-one loss of orientation on the distinguished pair
\(\{w,w^{-1}\}\) at fixed \(B\). The retained unoriented fixed-algebra datum
does not contain enough information to reconstruct which of these two cyclic
directions supplied it; no classification of every order-three automorphism with
that projector is being asserted.

There is also an honest orientation form. On \(B^\perp\), put

$$
\varpi_w(u,v):=\langle I_wu,v\rangle,
\qquad
\Omega_w:=\frac1{9!}\varpi_w^{\wedge9}.
\tag{OT10}
$$

Because \((B^\perp,I_w)\) has complex dimension nine,
...
One explicit quotient convention is
\(\Phi(z,A,B)=(z^3A,z^{-2}B)\). Its kernel is generated by
\((e^{i\pi/3},-I_2,e^{2\pi i/3}I_3)\), making the displayed
\(\mathbb Z_6\) precise.
...
\longrightarrow
\text{ }S(U(2)\times U(3))\text{ stabilizer}.}
\tag{OT16}
$$
...
## The internal grading is retained by an orientation lift

[[exceptional-gauge-realization/octonionic-slice-groupoid-and-orientation-torsor]] gives a smaller exact model
of the same two-sheeted grammar.
...
Put
\(K_u:=\operatorname{Stab}_{G_2}(\{u,-u\})
=N_{G_2}(SU(3)_u)\). Since
...
Thus an oriented lift may be locally definite while failing global descent.
The cocycle records the obstruction; it does not choose which sheet is factual
when the obstruction vanishes.

[[exceptional-gauge-realization/octonionic-clifford-completion-of-the-color-normal]] shows that this cover acts
nontrivially on an exact operator carrier. On
\(\mathcal H_{\mathbb O}
=\mathbb O\otimes_{\mathbb R}\mathbb C_{\mathrm{ext}}\), set
...
valued in the associated sign line, not as an ordinary endomorphism of the
coarse quotient. Its square, its unordered eigenspace pair, and the even trace
response do descend.

This no-go is scoped to the displayed
\(\operatorname{End}(\mathcal H_{\mathbb O})\)-valued family with its stated
deck identification. It does not prohibit a grading on some independently
constructed twisted carrier.

This is an exact algebraic model of an **internal grading as a discernible
relative to its reversal**, but it is not yet the observed chiral matter
spectrum. The two halves
...
Therefore the metric-independent statement is
...
Unlike an arbitrary squared-distance potential, (OT21) is supplied by the Jordan multiplication itself. Unlike the larger redundant constraint package in [[exceptional-gauge-realization/jordan-idempotency-and-the-stabilizer-gap]], it also has a canonical unit normal form once the trace metric is fixed. If a total-space metric is chosen to extend that trace metric and make the tangent/normal splitting orthogonal, the associated Hessian operator has the shorthand spectrum \(0^{(40)}\oplus1^{(4)}\).
...
\cong
(\mathbf3_{\mathrm{ad}},\mathbf1)_0
\oplus
(\mathbf1,\mathbf1)_0.
\tag{OT26}
$$

The entries in (OT26) are ordered as
\((SU(2),SU(3)_{\mathrm c})_{U(1)}\). The color and \(U(1)\) factors act
trivially. The clean four-dimensional normal gap therefore cannot by itself
supply a faithful response for the surviving gauge group.
...
2. **Asymmetry:** \(I_w\) and \(\Omega_w\) are odd under the antiunitary exchange \(w\leftrightarrow w^2\).
...
[[exceptional-gauge-realization/receipts/exceptional_normal_holonomy_receipt.py|The shared exceptional-normal receipt]] verifies the dimension and multiplicity ledger behind (OT24). Yokota's automorphism and centralizer and Baez--Schwahn's stabilizer remain cited theorem inputs.
