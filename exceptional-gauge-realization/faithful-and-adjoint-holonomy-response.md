...
[[exceptional-gauge-realization/exceptional-normal-holonomy-and-the-residual-gauge-form|The exceptional normal probe]] already gives \(Q_N=288w\) per color plaquette at every holonomy, not merely near identity. Thus (FA3) decomposes the existing faithful response:
\[
Q_N=288a+144|1-t|^2.
\tag{FA9}
\]
...
[[exceptional-gauge-realization/receipts/nonlinear_holonomy_probe_receipt.py|The finite holonomy receipt]] checks the exact decomposition, center values, matching identity Hessians, and quartic remainder for \(SU(2)\), \(SU(3)\), and \(SU(4)\). These checks certify the written finite-dimensional identities, not their missing physical realization.
