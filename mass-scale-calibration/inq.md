...
[[mass-scale-calibration/scale-torsor-and-the-global-local-gap-invariant|The scale torsor]] indexes an unpointed family with matched length and inverse-length parameters. Its active convention \(\ell\mapsto a\ell\), \(K\mapsto K/a\) preserves the dimensionless form \(\ell K\) after unitary transport. A passive enlargement of the length-unit basis transforms the numerical coefficients in the opposite direction. [[mass-scale-calibration/quantity-lines-and-conformal-scales|Quantity lines and conformal scales]] separates these actions from choosing a physical metric section; [[scale-score-connection/inq|the scale-score connection]] supplies the distinct domain and differentiation contract. The [[dilation-covariant-spectra/inq|dilation theorem]] rules out a nontrivial positive edge only when rescaling is an exact unitary symmetry of one fixed generator on one carrier.
