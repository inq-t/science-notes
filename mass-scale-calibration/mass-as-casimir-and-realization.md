...
[[mass-scale-calibration/hbar-clock-and-the-calibration-firewall]] strengthens this into a parameter-rescaling no-go: an abstract spectral generator does not become a physical frequency until its one-parameter action is independently soldered to a clock or length, and \(\hbar\) cannot supply that missing normalization.
...
so that \(\Delta_E\) is unchanged. Pointing this scale torsor expresses the answer in MeV; it is not what makes the ratio positive. [[contemporary-puzzles/yang-mills-mass-gap/register-audit]] uses this member--family distinction to locate the remaining Clay problem in the ultraviolet-to-infrared trajectory rather than in a contradiction between classical scale covariance and observed mass.
