...
There is a constructive use of that same kernel distinction. [[conditional-fisher-coercivity/gauge-reduced-patch-coercivity|Interior gauge completion]] adds local gauge-refresh defects to an already supplied equivariant auxiliary generator. The sum agrees exactly with the original generator on invariant observables while bounding its redundant complement. A fixed Wilson patch demonstrates why this matters: gauge-dependent slow modes can force its full refresh gap to zero without testing the invariant gap. The arbitrary refresh rate does not create invariant coercivity or physical energy.
