...
The existing [[channel-loss-and-recovery/preserving-expectation-loss|output-transgression theorem]] makes the warning sharper. If \(A\) is taken to be only the loss Hessian of one state-preserving expectation with its adapted BKM geometries, the retained minimum-lift transgression is identically zero. A general noisy channel can instead leave a positive retained-output cost with ordinary Fisher metrics, as the [[channel-loss-and-recovery/binary-channel-witness|binary-channel witness]] shows. Non-adapted comparisons, jointly transverse families and independently constructed whole/boundary responses are other possibilities. Each requires its own declared response; relabeling the canonical vertical loss does not supply one.
