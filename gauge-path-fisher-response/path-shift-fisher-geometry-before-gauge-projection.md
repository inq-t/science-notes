...
\tag{PF4}
\]
Finite deterministic energy satisfies Novikov's sufficient condition,
so the exponential has expectation one; almost-sure finite energy alone
would not normalize a general adapted exponential. Under \(\mu_k\), the stochastic term in the exponent has mean
...
analysis]], §1.5's Girsanov theorem and §13.5, “Right derivative,” with
its deterministic rotated-driver translation identity. Girsanov's
normalized-exponential hypothesis is discharged above. The source uses diffusion generator
...
[[gauge-boundary-frame-gluing/finite-gauss-gluing#Closing the parts first loses physical loops|premature regional
...
[[gauge-source-action-transport/joint-path-law-and-the-shared-boundary-action|The theta-path classification]]
...
[[gauge-path-fisher-response/two-sided-fisher-completion-and-the-neutral-carrier|Joint two-sided
...
The [[gauge-path-fisher-response/receipts/path_shift_fisher_receipt.py|path-shift Fisher receipt]]
...
Its [[gauge-path-fisher-response/receipts/path-shift-fisher-receipt-output.txt|recorded output]] is
