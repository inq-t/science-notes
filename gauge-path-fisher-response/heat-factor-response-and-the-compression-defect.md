...
[[gauge-path-fisher-response/shared-driver-response-and-the-nested-holonomy-clock|the nested-holonomy construction]], and \(J:L^2(\mu_T)\to L^2(\nu_{\alpha,\beta})\) is an isometry. The claim is a path-law identity, not only equality of endpoint heat kernels.
...
The scaled [[gauge-path-fisher-response/path-shift-fisher-geometry-before-gauge-projection|one-sided path-shift formula]] and independence give
\[
D(\nu_{k,l}\Vert\nu_{\alpha,\beta})
=\frac14\int_0^T
...
\tag{HF5}
\]
By contrast, [[gauge-path-fisher-response/two-sided-fisher-completion-and-the-neutral-carrier#The joint action has an actual state cost|the output entropy calculation]] gives
\[
D(\mu_{k,l}\Vert\mu_T)
=\frac14\int_0^T
...
[[gauge-path-fisher-response/two-sided-fisher-completion-and-the-neutral-carrier#The complete threshold is two, but two is not an eigenvalue|the heat-character calculation]] give
...
[[gauge-path-fisher-response/two-sided-fisher-completion-and-the-neutral-carrier#Conditional restart changes both operator and carrier|the loss caused by neutralizing a frozen future fiber]]. The full-cut law explains how its charged factors are retained.
...
not the tensor product of separately neutral regional carriers. It retains [[gauge-boundary-frame-gluing/finite-gauss-gluing#The whole is assembled from dual boundary charges|the complete pairing of contrary boundary representations]]. Close (HF18c) on the jointly invariant smooth cut cylinders. The cylinder bijection in (HF18a) is isometric for the displayed graph norms, so the closed cut domain maps exactly onto \(D(\mathcal E_\alpha^{s+t})\). In particular, its full semigroup obeys
\[
...
This distinction is detectable on [[gauge-path-fisher-response/path-shift-fisher-geometry-before-gauge-projection#The choice of handed action remains visible|the invariant three-holonomy determinant]]. At its stated rational SU(2) triple and times \(1,2,3\), cut at \(s=2\). The untwisted tensor-right response is \(67/625\); the endpoint correction restores the actual whole value \(481/625\), while the whole left value is \(193/625\). Thus replacing (HF18c) by the tensor sum of regional mixed clocks changes a concrete globally neutral response.
...
[[gauge-source-action-transport/source-action-transport-through-ordered-cuts|The retained-source cut]]
...
[[gauge-graph-source-response/haar-vertex-source-and-joint-gauge-response|The Haar-vertex graph source]]
