...
dictionary between (i) a Cauchy/Lorentzian line in energy, (ii) one-sided
exponential decay in time, and (iii) the contraction residue of a transfer
operator across a reflection wall. It does NOT establish reflection positivity
...
Checks
  ...
  C. 1d Ising transfer matrix (the finite debugging model of "residue of descent = gap"):
     ...
     gap -ln(lambda1/lambda0) must decrease monotonically in K and vanish as K -> infinity
     (the degenerating regime in which the check has something to fail).
  D. Numbers only: the pure-gauge gap length vs the causal grain, and the lattice-consistent
...
# ---- A. Lorentzian <-> one-sided exponential decay -------------------------
E0, G = 1.3, 0.4
...
# ---- C. 1d Ising transfer matrix: residue of descent = gap -----------------
K = 0.8
...
# the gap must shrink as the coupling grows and vanish in the degenerate limit
...
# ---- D. Numbers: pure-gauge gap vs causal grain (report, plus range regression) ----
...
out["D_gap_length_fm_range"] = [round(hbarc_MeVfm/m0pp_range[1], 4), round(hbarc_MeVfm/m0pp_range[0], 4)]
...
out["D_grain_over_gap_range"] = [round(lo, 1), round(hi, 1)]
