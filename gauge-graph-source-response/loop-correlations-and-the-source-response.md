# Loop Correlations and the Source Response

A smooth gauge-invariant loop weight changes the retained heat source's state and Fisher metric together. Conjugation symmetry reduces its entropy correction to an averaged endpoint Hessian, which determines a closed finite-graph response. Ordinary subdivision preserves the tilted state but changes that response, so the interaction and its comparison actions require a joint selection law.
...
Use [[gauge-graph-source-response/haar-vertex-source-and-joint-gauge-response|the Haar-vertex source]]
...
The scaled [[gauge-path-fisher-response/path-shift-fisher-geometry-before-gauge-projection|path-shift likelihood]]
has baseline score
\[
...
[[gauge-graph-source-response/local-interaction-cocycle-and-global-source-response|The local conditional-law construction]]
...
[[prepared-vacuum-fisher-comparison/physical-vacuum-lift-and-fisher-comparison|Supplying the actual physical vacuum]]
...
This is the [[general-causal-action/research-schema#Joint selection has two distinct tests|disguised-input test]]
...
[[gauge-source-action-transport/source-action-transport-through-ordered-cuts|Transporting the complete source]]
...
[[gauge-path-fisher-response/receipts/two_sided_fisher_receipt.py|The source-response receipt]]
