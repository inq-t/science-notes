...
There is no global density comparison with product \(m\); only a one-coordinate midpoint comparison after conditioning on the rest of the histories. Equation (TC13) alone does not give a finite-step physical transfer or complete midpoint-to-two-endpoint gap. [[smoothed-endpoint-response|Smoothed endpoint response]] supplies the additional joint Fisher estimate by distributing an initial-context change over an internal ramp and keeping an unchanged buffer. For the actual Wilson law its coefficient remains bounded through temporal refinement, and the same midpoint inequality then yields a complete two-ended floor. The [[strong-coupling-gap-and-continuum-crossover/wilson-to-hamiltonian-vacuum-limit|Hamiltonian limit]] supplies a separate physical return through the exact ground-state Dirichlet identity.
