...
If \(N=\log a\), \(T=T(N)\) follows a homogeneous FLRW trajectory, and this same stress sector is conserved so that \(\mathrm d\rho/\mathrm dN=-3(\rho+p)\), then [[causal-grain-cosmology/trace-residue-as-a-scale-cocycle]] makes this the local logarithmic-scale rate
...
[[causal-grain-cosmology/receipts/standard_model_trace_fossil_receipt.py|The arithmetic receipt]] reproduces the selected-row values, the endpoint residues, and the ideal-pion trace-shape calculation. [[causal-grain-cosmology/receipts/standard-model-trace-fossil-receipt-output.txt|Its recorded output]] also retains the muon shoulder and decay-rate negative controls; none supplies a wall-to-thermal map or a statistical exclusion.
