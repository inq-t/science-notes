...
The unit of the wall ledger looked like an arbitrary choice — nats against bits — and the reported matching-ratio fit looked like a measurement of an entropy. In the exact type-I product-edge model, the entropy bound can be retyped through the Watatani index; conditional on the channel model and reported fit, the resulting candidate lies above the discrete sub-4 Jones spectrum. This removes one possible rigidity obstruction but neither selects an index nor explains the value or continuity of the gravitational coupling.
...
## What the fit would constrain in the channel model
...
Under the channel-additivity and unit-rate assumptions, every rigid value predicts $\mathfrak R_c\ge1.4427$ and the reported fit interval $[0.941,1.088]$ excludes them. The same model excludes a maximally mixed qutrit from below, since it gives $\mathfrak R_c=1/\ln3=0.910$. Remaining candidates include $s_*\approx1$ nat on a nonmaximally mixed type-I cell of dimension $d\ge3$, or a suitable II$_1$-type inclusion satisfying
