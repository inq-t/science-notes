...
**5. The type-I edge model converts an entropy bound into an index bound.** **[CONDITIONAL ON THE CHANNEL MODEL AND A REPORTED FIT]** In the exact type-I product-edge/tracial model, the entropy-per-channel inferred from the reported matching-ratio fit, $s_*\in[0.92,1.06]$ nat, implies $\operatorname{Ind}_W(E_\tau)\ge e^{2s_*}>4$. Jones's theorem then excludes the discrete sub-4 ladder for this candidate. It does not select an index or explain the value or continuity of $G$. [[index-not-entropy]] states the assumptions, the ladder, and the kill conditions.
