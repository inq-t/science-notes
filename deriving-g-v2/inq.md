...
The first pass, [[deriving-value-of-g/inq|deriving the value of G]], established what a derivation must construct and proved no-go lemmas against shortcuts. This second pass reorganizes the question around one typed observation: the Unruh, Gibbons–Hawking, and Hawking-as-surface-gravity laws share a G-free KMS conversion pattern, while the gravitational normalization enters the source-to-geometry and ledger-to-area maps. Written in the wall's own units the first law contains no explicit G; G reappears in its presentation as metric area. "The value of G" is then a relational statement about the grain of matter rulers, and the no-gos of the first pass funnel into an independently normalized area density plus a same-tangent alignment theorem. The numerical diagnostics here exclude specified unscreened running laws and, conditionally on the channel model and reproduced direct background profile, the sub-4 Jones ladder. Claim labels follow [[program-core/axioms-and-principles#Status vocabulary|the programme status vocabulary]] and its composition rule; nothing below outranks its weakest load-bearing edge.
...
**5. The type-I edge model converts an entropy bound into an index bound.** **[CONDITIONAL ON THE CHANNEL MODEL AND A REPRODUCED BACKGROUND PROFILE]** In the exact type-I product-edge/tracial model, the conditional map $s_*=1/\mathfrak R_c$ sends the fully released 2025 direct unit-rate profile to $s_*=0.9861$ nat, with $\Delta\chi^2\le1$ range $[0.9175,1.0621]$. Jones's theorem then leaves the candidate above the discrete sub-4 ladder even at the wider $\Delta\chi^2\le3.84$ contour. The maximally mixed qutrit is only mildly disfavored: outside the narrower contour but inside the wider one. This does not select an index or explain the value or continuity of $G$. [[index-not-entropy]] states the assumptions, the ladder, and the kill conditions.
...
| Empirical or conditional exclusion | The declared unscreened live-running laws (LLR, factors 217 to 434; the displayed log ansatz, factor 2.3); the sub-4 rigid ladder only under the channel and reproduced-profile hypotheses; a maximal qutrit is mildly disfavored, not excluded at the wider contour |
