...
For a commutative readout context \(\mathcal D\subseteq\mathcal M\), restriction supplies

$$
\omega|_{\mathcal D}
\longleftrightarrow
\mu_{\omega,\mathcal D}
\in\operatorname{Prob}(X_{\mathcal D}),
\qquad
X_{\mathcal D}:=\operatorname{Spec}(\mathcal D).
$$

A fact requires an actual

$$
x\in X_{\mathcal D}
$$

or the corresponding character \(\chi_x\). No property of the measure alone selects \(x\). The conjecture must therefore construct more than decoherence or restriction.

## Instrument and record data

A minimal fact-forming model should specify

1. a context-selection rule for \(\mathcal D\);
2. a countably additive instrument \(\Delta\mapsto\mathcal I(\Delta)\) on measurable subsets \(\Delta\subseteq X_{\mathcal D}\), or a discrete family of completely positive operations \(\{\mathcal I_x\}\);
3. the conditional post-fact state associated with an obtained \(x\);
4. a record algebra \(\mathcal R\) in which the outcome is physically encoded; and
5. a one-sided evolution preserving the record for the relevant future domain.
