...
Equivalently, under a scale translation \(A\mapsto A+sI\), the squared response transforms by the character \(e^{(q-2p)s}\). It is scale-neutral precisely when \(q=2p\). This is a **common translation of invariant mean scale** and should be interpreted as address or theory-family covariance, not an exact unitary dilation symmetry inside one fixed gapped theory; [[dilation-covariant-spectra/inq#A gap obstructs exact same-carrier dilation covariance|the same-carrier dilation no-go]] forbids the latter. It is also distinct from the reciprocal frame or boost shift of the two directional addresses, under which their mean \(A\) is already fixed.
