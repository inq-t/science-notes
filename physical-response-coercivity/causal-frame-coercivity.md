...
**Status: [EXACT CONDITIONAL FRAME THEOREM; EXACT LOCAL-EXPECTATION NO-GO UNDER REEH--SCHLIEDER HYPOTHESES; STANDARD REAL LOCALIZED ENERGY COMPARISON; OPEN YANG--MILLS CONSTRUCTION].** The functional-analytic implications are proved below. [[physical-response-coercivity/localized-relative-entropy-and-the-energy-solder|Longo's finite-width bound]] supplies the energy comparison for real localized relative-entropy Hessians under explicit entropy and energy-form differentiability. A spectral conclusion additionally needs a positive Hermitian extension or compatible real structure on a complex Hamiltonian form core. No causal wall family, such extension, uniform regional lower frame, whole-to-local width map, or continuum Yang--Mills member satisfying the remaining premises has yet been constructed.
...
Equation (CF) is the exact algebraic meaning proposed for a **causal-frame lower bound**, or distinction grain: there is no normalized non-vacuum direction that becomes arbitrarily invisible to the entire physically admissible wall family. It is not a shortest distance. The analysis map has lower singular bound $\sqrt{\kappa_{\mathrm{fr}}}$. The dimensional grain $\lambda_*$ belongs to an [[the-grain-of-causal-scale/relational-grain-construction|object-relative entropy/scale calibration]]; it is not automatically a correlation length, localization width, or spatial pixel. Identifying $E_*=\hbar c/\lambda_*$ with a physical excitation scale requires a separate same-carrier matching theorem.
...
where $g(\Gamma)$ is graph girth and $\lambda_G$ is the smallest nonzero Casimir in the declared invariant-metric convention. The proof is representation theoretic: every nontrivial closed spin network contains a cycle, and a Wilson character on a shortest cycle saturates the bound. [[strong-coupling-gap-and-continuum-crossover/gauge-descent-flux-fisher-coercivity]] gives the theorem and its assumptions. This is kinematic gauge closure, not factivity and not a specifically three-dimensional knot effect.

The physical vacuum changes the norm from product Haar to $\nu_{a,L}=\psi_{0,a,L}^2\mu_{a,L}$. A global density-ratio transfer is exact but normally deteriorates exponentially with volume. The sharper target is local: a uniform conditional Poincare constant plus a Wasserstein--Dobrushin influence matrix of spectral radius below one gives a vacuum-weighted flux Poincare bound. This is an exact finite Haar benchmark for (P1), not a realization of the causal-wall family. [[global-local-response-reconstruction/vacuum-boundary-gluing-and-wall-response#The OS quotient factors exactly through a reflection interface|Reflection-Markov interface factorization]] now constructs the OS-to-interface unitary $B_r^{\mathrm{OS}}$ under explicit finite-regulator hypotheses. After infinite-depth vacuum preparation and uniqueness identify its reference line with $P_{0,r}$, the composite $D_{r,I}B_r^{\mathrm{OS}}(1-P_{0,r})$ is complex-linear and phase-sensitive; completeness would additionally require $\ker D_{r,I}=\mathbb C1$ on the gauge-reduced interface. A thick interface must still be mapped to the canonical transfer slice before this derivative can be identified with Kogut--Susskind electric flux. [[contemporary-puzzles/yang-mills-mass-gap/resolvent-logistic-scale-transform]] now supplies an exact complex-linear scale transform with coverage one after an independent positive interface operator with kernel \(\mathbb C1\) is chosen. What remains open is that operator's geometric selection and localization, any needed interface-to-slice identification, the mismatch between its log-Haar half-density carrier and the fixed weighted wall carrier, a uniform ceiling on its invariant scale center, the interacting-vacuum inequality, and the Casimir/continuum comparison.
