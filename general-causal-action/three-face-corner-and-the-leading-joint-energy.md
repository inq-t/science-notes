...
The Hamiltonian comparison restores complete recoupling when \(t\to1\), with the same physical kinetic calibration. Fix \(\varepsilon>0\), choose \(t=e^{-\varepsilon a_t}\), and put \(s_i=a_tg_i\). The fixed-spin Wilson expansion then gives
\[
...
\tag{JL18}
\]
Its free source-sector edge is \(4\varepsilon\), matching the Abelian fundamental clock. [[abelian-corner-and-the-joint-seam-energy|AC15]] evaluates the complete fixed-charge Abelian benchmark as \(\mathcal J_{U(1)}=-239/1080\). [[corner-hamiltonian-and-the-complete-fusion-response|CH12]] now gives \(\mathcal J_{SU(2)}=-299687/5885880\), using all five middle spin channels.

The high-spin remainder in (JL4) is no longer small in this weak-step regime. CH reinstates every contributing channel for (JL18), including its vacuum and disconnected terms; arbitrarily many spins are not required for that coefficient. The absolute reduction survives, but [[corner-response-and-the-failure-of-multiplicative-attenuation|the normalized connected gain]] is larger than the Abelian control after the two local reductions are accounted for. Thus a product of local suppression factors does not control the joint response. The full discrete-\(t\) function remains open. This is a finite spatial graph with a temporal Hamiltonian limit; [[strong-coupling-gap-and-continuum-crossover/inq|spatial continuum and volume-uniform control]] remain separate obligations.
