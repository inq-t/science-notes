...
The proof does not identify the \(h^2\) coefficient uniformly, give an \(h^4n^{39}\) spectral remainder, or prove the normalized susceptibility remainder \(h^4n^{41}\). Those require a sharper operator and source calculation. It also does not transport the isolated planar vacuum into a larger spatial region or supply the four-dimensional continuum scaling. The remaining step is a quantified nonlinear residual estimate with the same physical source, not another unquantified fixed-patch localization argument.
