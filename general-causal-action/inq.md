...
[[general-causal-action/sewn-overlap-and-conditional-clock|Sewn overlaps with conditional processing]] now give an explicit member: the same comparison determines a correlated state, discrepancy row and entire mixed clock once the processing rule is declared. Its binary gap is exactly a \(\operatorname{sech}^2\) profile, but tends to zero as comparison sharpness grows. The compact-group member and [[coarse-response-memory/correlated-interface-tangent|the certified Yang–Mills interface tangent]] therefore pose a concrete next comparison: preserve the complete joint response while constructing an unbounded refinement, rather than infer a uniform gap from the profile alone.
