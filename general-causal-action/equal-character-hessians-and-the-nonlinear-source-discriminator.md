...
[[weighted-character-gap-coefficient-and-the-spatial-soft-mode|The growing-patch coefficient]] and [[uniform-weighted-character-return-and-the-soft-gap|its actual uniform return]] extend the spectral contrast: it follows the closing harmonic frequency throughout the quantified confinement window. [[character-response-and-the-bulk-vacuum-normalization|The same variation of the closed trace]] has an extensive vacuum coefficient and a fixed normalized relation to this soft-gap response. [[character-memory-in-the-local-conditional-fourth-cumulant|The local conditional contrast]] has a positive bulk coefficient, and [[uniform-local-conditional-character-return|its actual uniform return]] now preserves that coefficient for the same bounded mark and complete exterior conditioning. This is a statement about the integrated fourth cumulant and its parameter derivative, not every conditional moment function or a continuum universality class.
