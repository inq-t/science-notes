...
Thus the next selector must supply and transport a whole-diagram background covariance \(G_{\mathcal D}\), derived from multiplication, the declared pairing and marked preparation, rather than reset it after each reduction. A genuine cycle readout \(T_\gamma\) would retain covariance \(T_\gamma\Pi(G_{\mathcal D},\mathscr R_{\mathcal D})T_\gamma^\dagger\); its survival and uniform physical rigidity remain to be proved. Conditioning establishes probability-law coherence under the stated maps, not the complete fixed-reference determinant and source normalization law. Those obligations, nontrivial multiplication transport and the physical transfer, vacuum sector and mass gap remain open.
