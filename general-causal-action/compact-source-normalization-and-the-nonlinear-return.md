...
[[uniform-planar-localization-and-the-first-physical-levels|Uniform planar localization]] supplies actual low-level separation on \(hn^{10}\le\eta\), and [[weighted-compact-source-return-on-growing-patches|the exact weighted source estimate]] replaces the global \(h^{-2}\) multiplication loss by a graph-norm bound. [[uniform-nonlinear-planar-gap-and-marked-response|Uniform nonlinear return]] now adds the actual operator, cutoff and centered Poisson estimates, proving fourth-order remainders \(Ch^4n^{21}\) and \(Ch^4n^{23}\) for the scaled gap and normalized susceptibility. These bounds resolve the stated growing-patch test. They supply neither a positive nonlinear mass term nor a fixed-coupling infinite-volume gap.
