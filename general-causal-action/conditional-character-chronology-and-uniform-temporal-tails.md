...
[[conditional-influence-diagonal-and-bulk-spectral-measure|The diagonal influence return]] now makes the spectral limitation explicit: its bulk density is proportional to \(\lambda^3\) near zero, with finite susceptibility but no positive spectral edge. For a central face, [[conditional-influence-soft-band-and-chronological-filters|the actual compact test]] supplies nonzero weight in a band of width \(O(1/(L+1))\) and an explicit normalized chronological filter with OI quotient of that order. These are fixed-scaled-duration statements; at fixed physical duration the same confinement trajectory drives the full-carrier quotient toward one.

The influence retains the complete exterior and the global insertion retains every face. Their joint temporal return must not be read as an autonomous one-face Markov law. A spatial composition still has to preserve [[two-slice-innovation-geometry/regional-innovation-and-exterior-information-balance|RI's exterior response]] and [[spatial-block-sewing-and-the-vacuum-cap-response|SB's caps and source exchange]]. One controlled matrix element supplies no positive floor for the complete physical source algebra, no fixed-coupling thermodynamic limit, and no four-dimensional Yang–Mills construction.
