...
[[conditional-character-chronology-and-uniform-temporal-tails|The subsequent chronological return]] transports the full conditional influence and centered character insertion through the original evolution. Its absolute time-integrated estimate improves the derivative remainder to \(C_Ih^3n^{15/2}\), and the contrast remainder to \(C_I|\varepsilon_2-\varepsilon_1|h^3n^{15/2}\), on the same confinement window. UL3 and UL22 retain the independent amplitude-derivative proof above; the sharper bound additionally uses the returned time profile.
