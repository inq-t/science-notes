...
This theorem returns source vectors and their actual equal-time normalizations. [[compact-chronology-on-finite-hermite-sources|HC]] supplies the separate chronological estimate; a fixed-support source is not automatically confined to one energy. [[compact-quadratic-carrier-and-the-pairing-range-return|The complete quadratic source map]] extends the centered argument to arbitrary signed growing matrices, paying for their small variances before returning mixed chronological forms. A bulk limit still requires a boundary-distance prescription. Neither construction identifies the inherited state with an independently prepared regional vacuum or supplies fixed-physical-time infinite-volume dynamics.
