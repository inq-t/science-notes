...
\(\kappa(\varepsilon_2-\varepsilon_1)k_L\), which tends to zero, while the absolute gaps themselves grow on the strong-confinement window. These statements concern the isolated planar carrier and the specified parameter interval. They do not supply a fixed-coupling thermodynamic limit, a four-dimensional continuum trajectory, or invariance of every nonlinear source statistic under this preparation change.
