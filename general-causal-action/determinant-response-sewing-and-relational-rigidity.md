...
The commutator candidate now has a finite joint-context test: two retained frames coupled to one preparation have a response depending on their relative axes. Nonparallel noncentral frames remove the traceless common nullspace in its \(M_2\) member, while a scalar null direction always survives. Neither fact identifies a physical vacuum or supplies a translation generator. The next construction must sew this same law across boundaries and derive its observable transfer, instead of declaring an internal commutator eigenvalue to be mass.
