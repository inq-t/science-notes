...
## Two-boundary rigidity: a sufficient stronger route
...
The earlier two-boundary proposal can be stated on this selected chronological law. Ground-transform the normalized transfer to \(P\) on the gauge-invariant \(L^2(\pi)\) carrier, equivalently the site-gauge orbit space. In auxiliary representatives \(d\pi=|\varphi|^2m\,d\Xi\); \(P\) is its actual one-step reversible Markov operator on that supported carrier. At a slab half-width \(\ell=k a_t\), \(k\in\mathbb N_{>0}\), let
\[
\delta_\ell f
=f(\Xi_0)-
...
**Sufficient repair criterion.** Suppose the boundary contractions and sewing relations provide an explicit repair \(\mathcal B_\ell\), assembled without a spectral inverse, such that on \(\mathscr H_{\rm frame}\ominus\mathbb C1\),
\[
...
\tag{DS10g}
\]
with \(C,\rho\) controlled at fixed positive physical \(\ell\) through the required limits. This is the concrete [[global-local-response-reconstruction/quantitative-descent-and-the-shape-of-a-gap|QD7–8]] criterion on the reflected source sector. Equations (DS10f)–(DS10g) would force \(H\ge(1-\rho)^2/(2\ell C^2)\) there. Neither positivity of the amplitude nor AS's bounded-source estimate proves the repair.

Two exact tests change the priority of this criterion. [[two-endpoint-recovery-and-a-rare-state-escape|RE1–22]] exhibits a three-state positive, injective transfer whose fixed-step energy gap grows while its two-ended recovery floor vanishes. That family is not continuously Markov-embeddable; it disproves the generic finite-step implication, not a bound for this determinant family or for every slab width. [[feature-number-grading-and-compact-reconstruction|FN1–14]] separately excludes repairs whose every branch passes through the global compact feature analysis and fixed finite number or ladder operations. Its centered neutral plaquette retains a degree-zero feature component, so raw feature number also misidentifies the vacuum. The compactness obstruction concerns undoing high-energy smoothing; it is not a physical soft-mode theorem. The two-boundary criterion remains sufficient, but is no longer the active additional law.

## Active candidate: oriented innovation transport

Retain the same \(P\), vacuum, source sector and physical half-width. Put \(A=P^k\), \(\mathscr H_0=\mathscr H_{\rm frame}\ominus\mathbb C1\), and use the actual past-to-middle innovation
\[
\delta_+f=f(\Xi_0)-(Af)(\Xi_{-k}),\qquad
R_+=\delta_+^*\delta_+=I-A^2.
\tag{DS10h}
\]
The innovation carrier is \(\mathscr K_0=\overline{\delta_+\mathscr H_0}\). [[oriented-innovation-and-finite-temporal-repair|The oriented construction]] owns the proofs. Its finite chronological word
\(\mathcal B_m=(I+A^2+\cdots+A^{2m-2})\delta_+^*\)
has \(\|\mathcal B_m\|\le\sqrt m\) and residual \(I-\mathcal B_m\delta_+=A^{2m}\). Unlike a feature-only reconstruction, it retains the unsmoothed innovation. The exact identity does not estimate that residual.

There is also an explicit transport word. Advance the two endpoints independently by \(A\):
\[
(\mathcal K_\parallel h)(x,y)
=\int A(x,dx')A(y,dy')h(x',y').
\]
The input endpoint law \(\mu_1(dx,dy)=\pi(dx)A(x,dy)\) becomes \(\mu_3(dx',dy')=\pi(dx')A^3(x',dy')\). Accordingly, this formula is typed as \(\mathcal K_\parallel=Q\iota\), where \(Q:L^2(\mu_3)\to L^2(\mu_1)\) is the actual conditional integration and \(\iota\) retains the same innovation function under the changed lag. On \(\mathscr K_0\), \(\|\iota\|\le\sqrt3\), and the resulting word is an exact contraction satisfying \(\mathcal K_\parallel\delta_+=\delta_+A\). No bounded inclusion of the entire pair \(L^2\) spaces is asserted.

**Active conjecture.** For this concrete \(\mathcal K=\mathcal K_\parallel\), the non-Abelian boundary composition law admits a strict positive remainder, constructed from its actual conditional source geometry:
\[
\delta_+A=\mathcal K\delta_+,\qquad
\eta^2I-\mathcal K^*\mathcal K
=\sum_\alpha\mathcal F_\alpha^*\mathcal F_\alpha,
\qquad 0<\eta<1.
\tag{DS10i}
\]
...
\[
R_+^2-(1-\eta^2)R_+
=\sum_\alpha(\mathcal F_\alpha\delta_+)^*
...
[[spatial-block-sewing-and-the-vacuum-cap-response|The spatial block calculation]] now isolates the exact interaction left after conditioning a separator history. The local action separates, but the two outer vacuum caps retain the factors \(c_-c_+\), where \(c=\sqrt{\pi(a,b\mid z)/[\pi(a\mid z)\pi(b\mid z)]}\). Integrating an adjacent block inserts a sourced conditional message; its full normalization and the mixed covariance of the two cap scores remain. A rectangular log-oscillation estimate controls conditional block correlation. A corresponding bound on the actual replica integration also retains the spatial spread of \(Af\); it is not a bound for a substituted sampler.

[[seam-coupling-response-and-the-vacuum-cap|The seam response]] forces these vacuum changes from the same temporal law. For \(T_s=e^{-sV/2}Te^{-sV/2}\), put \(V_{c,s}=V-\pi_sV\) and \(w_s=-(I-P_s)^{-1}V_{c,s}\) at fixed graph. Then
\[
\partial_s\log p_s(x,y)=w_s(y)-P_sw_s(x),\qquad
\|\delta_sw_s\|_{\pi_sP_s}^2=\partial_s^2\log\lambda_s.
\]
The inverse abbreviates a proved fixed-graph chronological series; it is not used as a uniform-gap certificate. Thus the seam's transition score, its actual innovation and its closed-normalization susceptibility are one forced response. Its temporal tail and interface dependence remain to be controlled.

[[adjacent-wilson-plaquette-and-the-chronological-surplus|The actual adjacent-plaquette test]] now evaluates that change. In the two-square \(SU(2)\) Wilson transfer, adding the right magnetic potential leaves the full left raw-link marginal Haar and the shared-edge-conditioned vacuum cap trivial. The shared-edge source product splits into singlet and triplet channels with fusion weights \(1/4\) and \(3/4\); their temporal multipliers determine the evolved source's coefficients, and its actual three-slab leakage is strictly nonzero at the first allowed order. The exact correlation derivatives retain both vacuum caps and the changing transfer normalization.

The test rejects monotone reinforcement by each seam. The left source's normalized surplus has opposite second-order signs at small and large temporal coupling. More decisively, the complete finite physical gap decreases for every fixed temporal coupling and sufficiently small positive right seam, including where that fixed source's quotient improves. The [[finite-seam-response-and-the-two-spin-surplus|finite seam-sign control]] separates the same two conclusions in closed form. Neither result excludes a positive global surplus; both require its proof to retain spectral redistribution and any negative seam contributions.

[[three-face-corner-and-the-leading-joint-energy|The three-face corner]] now has an evaluated joint response. At small fundamental temporal multiplier \(t\), the complete central-odd sector obeys \(\partial_{22}E_-=-t^2/(4a_t)+O(t^4/a_t)\). The fixed source's joint quotient coefficient is also negative. Three length-six channels and a uniform bound on every omitted spin prove the result. The outer-loop overlap \(1/4\) fixes its mixed term; the vacuum and disconnected eigenvalue contacts are retained.

[[abelian-corner-and-the-joint-seam-energy|The Abelian control]] has the same joint sign under the declared fundamental-multiplier, source-variance and clock calibration. Duplicating its shared seam edge removes the connected energy, so mere nonfactorization is an incidence effect already present in a commuting theory. The non-Abelian leading magnitude is one quarter of the Abelian value. This suggests a candidate mechanism within oriented innovation transport: normalized fusion could limit accumulated energy-lowering dressing. Its elementary factors are established boundary contractions, not an additional axiom; controlling their complete sum remains conjectural.

[[corner-hamiltonian-and-the-complete-fusion-response|The complete Hamiltonian test]] now gives \(\epsilon^3\partial_{22}E_-=-299687/5885880\), with five middle channels and every normalization term retained. The absolute reduction survives. However, [[corner-response-and-the-failure-of-multiplicative-attenuation|the locally normalized connected gain]] exceeds its Abelian counterpart by \(299687/239239>1\). This rejects multiplying the two measured local attenuation factors to bound the joint correction. The broader innovation conjecture survives; that particular composition mechanism does not.

[[strong-seam-resolvent-and-the-physical-rotor|The complete strong-seam return]] is now proved at fixed \(\epsilon\): the physical resolvent tends to the central rotor with fundamental energy \(10\epsilon/3\), including all additional modes and arbitrarily large representations. Its exact vacuum isometry preserves Haar measure. A derivative corrector attains the incidence-selected lower bound, while equivariant harmonic separation expels the fast complement. Bounded continuous source histories and a central magnetic face of fixed strength also return. [[abelian-corner-and-the-strong-seam-return|The Abelian comparison]] has the matching flux-sector coefficient.

This finite remainder does not give a size-independent composition constant. [[three-face-confinement-and-the-coupling-path|Comparable confinement of all three faces]] has a different asymptotic scale. [[planar-patch-confinement-and-the-spatial-soft-mode|The planar-patch theorem]] proves that the complete physical harmonic coefficient is \(2\sqrt{4-4\cos(\pi/(L+1))}\), which tends to zero as the patch grows. Gauss averaging still retains its quadratic scalar source. This rules out a volume-uniform positive coefficient multiplying \(\sqrt{\kappa g}\) in that strong-confinement family; it does not decide the fixed-coupling infinite-volume gap.

[[compact-source-normalization-and-the-nonlinear-return|The actual nonlinear source expansion]] retains the changing vacuum and all operator jets. Its [[nonlinear-scalar-source-and-the-vacuum-response-coefficient|first normalized coefficient]] separates the gap shift from source leakage. [[four-face-gap-shift-and-the-complete-source-response|The complete four-face test]] now gives a negative true gap correction together with a decreasing normalized susceptibility of the original source. Virtual cubic transitions reverse the positive direct energy correction. [[four-face-oriented-source-and-the-gap-following-probe|A fixed oriented counterprobe]] removes the leading leakage and tracks the negative gap correction with the same Hamiltonian, vacuum and clock. Thus the full source carrier contains an explicit test against apparent reinforcement inferred from one readout.

[[uniform-planar-localization-and-the-first-physical-levels|Uniform planar localization]] now proves a coarse return of the actual first physical levels throughout the proposed growing-patch window. The complete Gauss carrier retains a gap proportional to the harmonic \(L^{-1}\) coefficient in scaled units there; the absolute gap grows on this rapidly confining trajectory. [[weighted-compact-source-return-on-growing-patches|An exact compact operator estimate]] also controls source multiplication through the Hamiltonian, removing the crude \(h^{-2}\) loss.

[[uniform-nonlinear-planar-gap-and-marked-response|Uniform nonlinear return]] resolves the fourth-order marked-well test with stronger remainders \(Ch^4(L+1)^{21}\) and \(Ch^4(L+1)^{23}\). These terms preserve harmonic softening throughout the confinement window. [[compact-regional-covariance-and-susceptibility-return|The fixed-profile compact return]] now retains inherited regional covariance and finite local susceptibility. [[vacuum-hellinger-return-and-regional-conditional-projections|The actual conditional operator]] also has a positive one-face semigroup defect, equal to the information transported outside that region. [[regional-innovation-and-exterior-information-balance|The exact regional innovation balance]] determines how this channel enters the original response and its squared surplus. [[uniform-collar-capture-and-the-local-gap-limitation|Uniform collar capture]] now bounds its omitted fraction on the complete one-face radial harmonic algebra, with a compact return for the prescribed quadratic. Even the complete local harmonic OI floor stays positive while the global floor closes. [[finite-pairing-range-and-the-harmonic-innovation-floor|The range test]] now quantifies the escape: the quadratic floor is of order \(\min\{1,t/(R+1)\}\). [[compact-quadratic-carrier-and-the-pairing-range-return|The actual quadratic source map]] preserves this behavior for all signed matrices in its confinement window. [[four-face-diagonal-sources-and-the-relational-schur-defect|The four-face Schur test]] now resolves the Gaussian old/new separation. [[four-face-optimal-quadratic-response-and-parity|Its optimal full-quadratic response]] retains the apparent short-duration hardening, but [[four-face-oriented-source-extension-and-the-schur-surplus|two oriented cubic directions]] remove the leakage and return the negative actual gap correction. The generalized root includes the changing innovation norm; an unnormalized Schur coefficient is not a physical energy shift.

[[four-face-oriented-normal-form-and-the-universal-source-lift|The universal odd normal form]] now derives the entire quadratic source lift. [[four-face-source-products-and-the-oriented-contact|Its product test]] forces the coefficient \(4/7\) of an oriented cubic contact. [[oriented-source-products-and-the-compact-contact-return|The compact comparison]] detects that contact in the actual vacuum, with the composite's strength-dependent coefficient stated explicitly. [[oriented-source-lift-and-the-positive-comparison-obstruction|Its sign-indefinite same-source defect]] rules out ordinary positive conditional averaging as the complete multiplier lift. The operator commutator obeys Leibniz, and its truncated lift preserves products through first order. The lost differential information must therefore be retained in a proposed source reduction.

The subordinate construction within OI is **source sewing by the Lie bracket**. [[weighted-character-scale-and-bounded-lie-sources|The weighted scale]] and [[all-group-oriented-kinetic-jet-and-source-lift|actual all-group incidence calculation]] give the same normalized four-face first contact, with the structure-constant norm retained. [[regional-conditional-projection-and-the-vacuum-score|The actual projection tangent]] now follows from weighted fixed-patch vacuum control. [[two-face-source-access-and-the-normal-form-obstruction|Its five-by-five comparison]] proves that no invariant odd kinetic normal form can also match the full fixed one- and two-face projection inventory. The embedding response is therefore necessary; removing a resonant convention does not remove it.

[[regional-vacuum-score-and-the-three-face-orientation|The actual score]] has zero one- and two-face marginal projections and zero neutral scalar exchange between overlapping pairs, while a three-face orientation is nonzero. [[covariant-boundary-source-and-the-oriented-return|Its covariant boundary source]] retains a double bracket and a linear Casimir response fixed by the vacuum. [[four-face-covariant-route-response|The nonnested comparison]] now computes its direct and ordered returns with both score insertions. [[conditional-boundary-translation-and-source-products|The retained boundary message]] restores the original source and its products through either elimination order: a conditional mean shift carries the first score, and the inherited covariance supplies the product contractions. The same calculation rejects exact Gaussian promotion of all four conditional shifts on the unbounded harmonic carrier, because it forces a nonnormalizable cubic joint weight. The actual higher-order vacuum and chronology must therefore remain. The complete-source estimate and continuum crossover in [[conditional-vacuum-rigidity-and-the-physical-gap|CV8–14]] remain open.

[[covariant-source-memory-and-the-first-chronological-response|The actual chronological test]] gives a nonzero mixed exterior coefficient on physical scalar radius and Cartan-triple sources, even though their diagonal first coefficients vanish. [[radius-orientation-chronological-gram-and-the-source-pencil|The complete chronological Gram calculation]] now evaluates their mixed covariance at every fixed scaled duration and proves a sign change. [[radius-orientation-source-extension-and-the-innovation-floor|Its generalized source pencil]] lowers the actual radius-only quotient at second order for short fixed duration, with the changing innovation denominator retained. This relative source-extension comparison does not require the unknown absolute second-order radius shift.

[[logarithmic-vacuum-curvature-and-conditional-cumulants|The next logarithmic vacuum score]] is an even polynomial of degree at most four and determines the second conditional correction. A forced fourth cumulant distinguishes it from Gaussian translation; the original bounded marks also retain their cubic coordinate contact. [[equal-character-hessians-and-the-nonlinear-source-discriminator|A fixed-representation preparation family]] keeps the quadratic scale and complete first-order data unchanged while altering a gauge-invariant conditional fourth cumulant and the actual finite physical gap at second order. The varied weight is an explicit part of the full preparation. Source sewing by the Lie bracket must retain that character data; its quadratic reduction does not determine the nonlinear spectrum.

[[weighted-character-gap-coefficient-and-the-spatial-soft-mode|The spatial coefficient]] and [[uniform-weighted-character-return-and-the-soft-gap|its actual weighted return]] now prove that this character-dependent gap contrast follows the closing harmonic frequency on the quantified planar confinement window. Its normalized derivative tends to \(-7c_\infty/6\), with the original physical clock retained. [[character-response-and-the-bulk-vacuum-normalization|The same variation of the closed normalization]] has an extensive vacuum coefficient; the two normalized actual responses have ratio \(4/(3c_\infty)\) in that window. This relation retains the declared additive normalization and does not select a cosmic source or remove the possible scalar character.

[[character-memory-in-the-local-conditional-fourth-cumulant|The conditional source test]] gives a positive bulk fourth-cumulant parameter coefficient with the same original mark. [[uniform-local-conditional-character-return|Its actual uniform theorem]] now retains that coefficient throughout the confinement window. [[conditional-replica-cumulants-and-amplitude-stability|The four-replica representation]] controls all centering and moment products as one amplitude functional; [[magnetic-parameter-tangents-and-uniform-vacuum-control|the normalized vacuum tangent]] transports its parameter derivative without an inverse marginal-density bound.

[[conditional-cumulant-influence-and-the-original-chronology|The corresponding influence observable]] gives an exact mixed reduced-resolvent response under the original Hamiltonian. It depends on the full retained exterior, even though its underlying mark belongs to one face. [[conditional-character-chronology-and-uniform-temporal-tails|The full actual profile]] returns uniformly over scaled time and in absolute time integral. [[conditional-influence-diagonal-and-bulk-spectral-measure|Its diagonal spectral measure]] reaches zero scaled energy despite finite susceptibility. For a central face, [[conditional-influence-soft-band-and-chronological-filters|the actual soft-band test]] gives explicitly normalized chronological filters with OI quotient of order \(1/(L+1)\). A positive conditional character coefficient therefore does not repair the scaled soft source family. At fixed physical duration this rapidly confining trajectory has a floor tending to one, so the four-dimensional physical return remains undecided.

[[closed-cube-raw-edge-kinetic-sewing|The complete cube kinetic law]] now retains all twelve raw derivatives and the dependent sixth face. [[closed-cube-dependent-face-and-oriented-source-jet|Its ordered bracket mark]] has a fixed signed pairwise response absent after Abelianization. [[closed-cube-bracket-source-and-the-actual-vacuum-return|The actual compact source theorem]] returns that response and its chronology; the centered aggregate reaches the ordinary lowest neutral harmonic energy four. Its positive mean is consequently not an additional gap estimate. This develops the same subordinate source-sewing construction within OI.

[[two-cube-shared-face-and-the-original-kinetic-law|The shared-cell construction]] now retains nine independent loops, two compatible closures and one copy of every electric edge and face cost. [[two-cube-temporal-sewing-and-the-actual-joint-source|The same temporal sewing]] preserves the global vacuum caps and proves that attaching the interior cost strictly decreases a fixed joint bracket mean, although the full physical gap increases between the endpoints. The harmonic gap change occurs already in the Abelian incidence control.

[[two-cube-interior-cost-and-the-joint-bracket-response|The fully attached source]] misses the lowest physical modes at harmonic order, and a vanishing spectral weight changes its cyclic edge without a discontinuous operator gap. Actual profile return does not exclude smaller nonlinear weight at those modes. The next constitutive test is the actual lowest-cluster projection of this fixed bracket source, with the full prepared character and original clock retained. Local positive bracket activity and consistent elimination alone have not supplied the OI bound.

[[paired-boundary-return-and-the-same-wall-identity|Paired-boundary return]] also resolves the modular comparison: each same-boundary retraction is the identity, while the opposite-boundary return is \(P^*P\). Its prescribed-source entropy Hessian gives the existing innovation. A nested KMS wall return is a distinct possible observable, with a different metric from the relative-entropy BKM Hessian; it has not replaced the selected chronological law.
