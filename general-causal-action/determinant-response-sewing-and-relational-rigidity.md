...
The next decisive test is the complete change of \(V_Q-C_{\rm lag}\) under one adjacent Wilson-block seam deformation. Use the derived history score, both cap variations and \(\partial_sP_s^k\) to calculate the first nonzero connected source term, including the spread of \(Af\). Keep the charged boundary frames until the joint gauge projection. Determine whether the cap contribution and source spread can be absorbed by a finite actual boundary-word surplus with an interface-dependent constant. A bare local action bound does not suffice: the finite two-spin control shows that one bounded seam can have arbitrarily large vacuum cap oscillation when its chronology slows.
