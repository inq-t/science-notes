...
The next decisive test is [[three-face-corner-and-joint-seam-response|the three-face corner]]. The two magnetic seams share a third edge, so their joint response cannot be replaced by two independently returned loop kernels. Compute the first allowed connected coefficient, of order \(s_1^2s_2^2\), for the complete central-odd sector's energy edge and compare it with the fixed source's normalized surplus. Subtract both one-seam contributions and the actual vacuum normalization. The exact graph, parity constraints and finite spin bounds make this a finite algebraic calculation; its value and sign remain open. The active conjecture concerns a bound on the whole innovation carrier through physical refinement, not positivity of every individual seam or every selected source response.
