...
[[character-memory-in-the-local-conditional-fourth-cumulant|The conditional source test]] separately gives a positive bulk fourth-cumulant coefficient with the same original mark. Its actual theorem remains fixed-patch, so the next source-sewing estimate must control its conditional products uniformly through spatial growth. The complete-source OI bound and four-dimensional physical return remain open; the gap contrast alone supplies neither that bound nor a source-erasure theorem.
