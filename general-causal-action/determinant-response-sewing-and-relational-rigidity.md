...
The [[closed-cube-face-words-and-the-prepared-amplitude#A closed boundary does not distinguish an ambient bulk|next shared-cell test]] joins two cubes, retaining nine independent loops and two compatible cell relations. The shared face and its four electric edges must be counted once, with complete marked amplitudes and actual vacuum caps. A single closed boundary has an identical spherical realization; the interior plaquette's additional incidence is the next discriminating input. Its joint response, including a control omitting only the shared magnetic cost, must be calculated before any bulk rigidity is inferred.
