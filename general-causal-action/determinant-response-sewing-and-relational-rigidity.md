...
[[uniform-nonlinear-planar-gap-and-marked-response|Uniform nonlinear return]] resolves the fourth-order marked-well test with stronger remainders \(Ch^4(L+1)^{21}\) and \(Ch^4(L+1)^{23}\). The actual operator remainder, compact cutoff and centered Poisson solution retain the same vacuum, source and clock. These terms preserve the harmonic softening throughout the proved confinement window. [[inherited-planar-vacuum-and-the-regional-time-law|The regional harmonic benchmark]] now proves that an independently prepared block misses inherited chronological memory, including a neutral-source semigroup defect. The next test carries that inherited marked history into the actual compact law. The actual conditional covariance and mixed susceptibility in [[conditional-vacuum-rigidity-and-the-physical-gap|CV8–14]] must still travel through assembly and controlled crossover.
