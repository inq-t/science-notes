...
All these estimates concern a fixed patch, group and finite source family in the declared units. The value of \(\gamma\) comes from \(u/\Omega\), so adding a resonant source generator with \(N\Omega=0\) cannot retune it. [[four-face-covariant-route-response|The nonnested four-face comparison]] now computes its direct and ordered returns. [[conditional-boundary-translation-and-source-products|Retaining the conditional boundary message]] recovers the original source and its products through either elimination order, while ruling out an exact nonlinear Gaussian closure. [[two-face-source-access-and-the-normal-form-obstruction|The access obstruction]] remains: this is the actual conditional channel with its moving embedding. The [[spatial-block-sewing-and-the-vacuum-cap-response|vacuum-cap, source-exchange and changed-history terms]] and their control through the required physical limits remain open.
