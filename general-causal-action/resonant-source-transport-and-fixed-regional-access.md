...
The next construction is a square of actual source maps: compute the first transport of the inherited face and overlapping-region conditional projections under the same \(S+\sum_m\theta_mN_m\), and compare it with the projection obtained from the actual compact vacuum and fixed prepared face words. Their difference is a source-access discrepancy to retain or resolve through sewing. No positive conditional variance here is identified with a new Hamiltonian term or an OI gap bound; [[spatial-block-sewing-and-the-vacuum-cap-response|the vacuum caps, source exchange and changed-history lag]] still belong to that estimate.
