...
Three orthogonal plaquette faces meeting at one cubic corner give a finite test of genuinely overlapping seam response. Each added face shares an edge with the source face, and the two added faces share a third edge. The first possible joint response is fourth order by exact center symmetries. Both the complete source-face odd-sector energy and a fixed source's normalized surplus can be differentiated in finitely many exact spin sectors, without replacing the shared-edge Gauss law or resetting a compressed path kernel. Linked calculations prove negative joint coefficients at small temporal coupling and evaluate the exact Hamiltonian energy coefficient; the full discrete-coupling dependence remains open.

**Status: exact finite graph, symmetry and perturbative closure; evaluated small-coupling and Hamiltonian joint responses; open full discrete coefficient and physical-limit estimate.** [[adjacent-wilson-plaquette-and-the-chronological-surplus|The adjacent-plaquette calculation]] shows that an improving source quotient can coexist with a decreasing physical gap. This test keeps the complete relevant sector alongside that quotient. [[three-face-corner-and-the-leading-joint-energy|The leading joint-energy calculation]] and [[corner-hamiltonian-and-the-complete-fusion-response|the complete Hamiltonian response]] own the evaluations. Its three-dimensional spatial incidence differs from [[three-cell-incidence-and-shared-path-motion|the three-cell row]].
...
[[corner-hamiltonian-and-the-complete-fusion-response|The matched temporal Hamiltonian calculation]] now gives \(\epsilon^3\partial_{g_1}^2\partial_{g_2}^2E_-=-299687/5885880\), with \(t=e^{-\epsilon a_t}\), \(s_i=a_tg_i\). It retains all five contributing middle channels and [[quartic-seam-eigenvalues-and-the-generalized-pencil|all quartic normalization contacts]]. Its absolute Abelian comparison is reduced, but [[corner-response-and-the-failure-of-multiplicative-attenuation|the product of the two local reduction factors]] does not bound the joint response. Splitting the shared edge makes the quartic energy coefficient zero for both groups. The full discrete-\(t\) coefficient remains open; no uniform spatial or continuum conclusion follows from these finite results.
