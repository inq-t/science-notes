...
\tag{TM15}
\]
Evaluating these finite sums and comparing the actual generalized source pencil \(\mathcal N_h-\lambda\mathcal G_h\) is the next discriminator. Its Schur comparison must retain \(\mathcal G_h\), the diagonal response jets and actual centering; it cannot substitute the eigenvalues of an unnormalized exterior matrix.
