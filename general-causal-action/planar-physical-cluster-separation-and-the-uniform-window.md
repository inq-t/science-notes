...
The compact proof of (PS13) is supplied by UP's lower and upper min–max comparison. Spectral isolation alone does not establish GW's fourth-order marked remainder: the [[weighted-compact-source-return-on-growing-patches|weighted source estimate]] retains actual centering and variance normalization, while the nonlinear operator residual still needs control. The harmonic counting and separation here supply the spectral factors in that estimate.
