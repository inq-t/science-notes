...
The next spatial test can therefore follow the actual channels \(L_B(t)\) under nested reveals and quantify their mixed boundary response. The required strict OI bound must still control the complete response ratio through the physical limits. The exact positive budget specifies what sewing must preserve; it does not supply that missing estimate.
