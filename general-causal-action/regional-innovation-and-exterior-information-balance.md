...
[[uniform-collar-capture-and-the-local-gap-limitation|The collar test]] now bounds the omitted fraction by \(C/w^2\) on the complete one-face radial harmonic algebra, with a uniform compact return for the prescribed quadratic away from zero scaled time. It also proves that complete local coercivity can coexist with a closing global floor. The next test retains the mixed source matrix as support grows. The required strict OI bound must still control the complete response ratio through the physical limits. The exact positive budget specifies what sewing must preserve; it does not supply that missing estimate.
