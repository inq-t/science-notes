...
Source composition must therefore distinguish fixed face access, the moving amplitude projection, and the oriented three-face union. [[covariant-boundary-source-and-the-oriented-return|The covariant boundary return]] retains an adjoint pair index and derives a nonzero nested actual conditional response from this same \(\gamma_{abc}\). [[resonant-source-transport-and-fixed-regional-access|RG]] concerns a different first-order freedom: changing a normal-form source lift while retaining its physical clock. The cancellations here do not license keeping the old regional projection when that embedding moves, and do not prove that a single normal-form choice is compatible with every overlapping region.
