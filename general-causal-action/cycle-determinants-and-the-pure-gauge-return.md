...
No uniform volume estimate at fixed copy count is claimed in (CY8): \(M\) appears in (CY7). [[general-causal-action/cofinal-determinant-and-wilson-source-equivalence|The cofinal extension]] chooses \(\nu\) to dominate that factor on growing boxes and controls specified renormalized source products. It does not prove existence, nontriviality or a physical gap of the continuum Wilson theory. In the four-dimensional compact-simple Yang–Mills case, that construction remains the Clay target. The present result only shows that a blanket obstruction to a pure-gauge return from Gaussian auxiliaries would be too strong.
