...
The next executable bulk test is two cubes sharing one face. There are \(16-4=12\) vertices, \(24-4=20\) edges, \(12-1=11\) distinct plaquettes and \(20-12+1=9\) independent graph cycles. The two ordered cell relations must be retained with compatible connectors. The ten exterior faces form a sphere, while the shared plaquette supplies an additional incidence: each of its four edges belongs to three distinct plaquettes. That incidence is absent from the single boundary surface.

Sew the two cells with the shared face and its four edge kinetic terms counted once. Adding the two complete cell Hamiltonians would double those terms. An explicit factor allocation, or its equivalent full-amplitude gluing, must preserve the original joint marks and vacuum caps before comparing the two cell-elimination orders. The new test is a marked response depending on that shared interior incidence. Its stated control omits only the shared plaquette's magnetic cost, retaining its word as the same source mark and all twenty electric edges. Elimination-order agreement alone remains integration consistency.

[[reciprocal-coefficients-and-the-field-gap-test|FG]] already shows that linear incidence constraints alone permit soft modes. Full-amplitude integration, as in [[holonomy-state-refinement/overlap-kernels-and-face-refinement|OF15–18]], does not select the group, preparation or clock. Any shared-cell gap test must retain [[regional-innovation-and-exterior-information-balance|RI's inherited exterior response]] and [[spatial-block-sewing-and-the-vacuum-cap-response|SB's temporal vacuum caps]], rather than infer coercivity from the closure identity.
