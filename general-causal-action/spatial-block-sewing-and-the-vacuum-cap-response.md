...
The next calculation is the seam-coupling experiment of [[seam-coupling-response-and-the-vacuum-cap|SV13–16]], on the same Wilson transfer and with a fixed regional neutral source. Vary the declared crossing potential and compute the cap score, the transported \(Af\), and its leakage \(d\) before comparing (SB15). At a genuinely factorized control, its product-mode response is explicitly computable; factorization is not assumed for the actual gauge cut. Retain the complete source amplitude (SB4), the before/after-\(Q\) correction (SB13), and the \(\mu_1\)-to-\(\mu_3\) cost. Whether non-Abelian sewing supplies a positive surplus that survives volume and continuum limits remains the test.
