...
[[inherited-planar-vacuum-and-the-regional-time-law|The planar harmonic regional law]] gives an exact acceptance test for retaining this history. Its covariance is \((A^{1/2}e^{-|t|A^{1/2}})_{BB}\), and a neutral one-face source detects the failure of the regional two-time kernels to form a semigroup. In the bulk limit, a fixed local quadratic source has a power-law tail despite finite susceptibility. An independent regional oscillator therefore cannot replace the inherited history even after matching one-time data. The next compact-theory comparison must retain this temporal response as well as the equal-time vacuum cap.
