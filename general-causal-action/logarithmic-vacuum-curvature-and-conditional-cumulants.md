...
[[equal-character-hessians-and-the-nonlinear-source-discriminator|The equal-Hessian preparation family]] keeps the representation, total weight, quadratic index and original GM source fixed while changing the quartic cost. Its invariant conditional cumulant separates the actual preparations at order \(h^2\), although their first-order data agree. [[character-memory-in-the-local-conditional-fourth-cumulant|The parameter-contrast calculation on growing patches]] cancels the common kinetic, first-score and source-coordinate terms directly. [[uniform-local-conditional-character-return|The integrated conditional theorem]] now preserves its positive bulk coefficient on the compact confinement window. [[conditional-replica-cumulants-and-amplitude-stability|The replica representation]] controls the nonlinear conditional products without inverse marginal-density bounds. [[conditional-cumulant-influence-and-the-original-chronology|Its influence observable]] retains the moving conditional means, covariance and marginal in an exact response under the original chronology.
