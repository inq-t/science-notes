...
**Status: proved fixed-patch actual parameter contrast and sequential bulk coefficient limit.** [[equal-character-hessians-and-the-nonlinear-source-discriminator|EH]] fixes the preparation and source; [[weighted-character-gap-coefficient-and-the-spatial-soft-mode|WS]] owns the gap comparison. The argument below extends EH's conditional contrast without calculating the common second kinetic or vacuum coefficient. [[uniform-local-conditional-character-return|The uniform conditional theorem]] separately supplies the actual growing-patch return.
...
[[uniform-local-conditional-character-return|The actual uniform return]] now controls the integrated contrast in LCM8 with error \(C_I|\varepsilon_2-\varepsilon_1|h^3(L+1)^{17/2}\) on \(h(L+1)^{10}\le\eta_I\). It uses [[conditional-replica-cumulants-and-amplitude-stability|four conditional replicas]] and [[magnetic-parameter-tangents-and-uniform-vacuum-control|the differentiated vacuum]], keeping the original bounded mark and full exterior. Thus LCM12 also holds along that simultaneous trajectory. [[conditional-cumulant-influence-and-the-original-chronology|The influence calculation]] places the parameter response in the original chronological susceptibility. A uniform return of its complete temporal kernel remains a separate test. Any stronger claim that the character input disappears under refinement must specify its retained source algebra and complete sourced boundary map, as in [[holonomy-state-refinement/overlap-kernels-and-face-refinement|OF15–18]].
