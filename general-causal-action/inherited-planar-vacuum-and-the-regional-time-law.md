...
The acceptance test is to return (IR3)–(IR5), including mixed sources, when eliminating an exterior. [[compact-regional-covariance-and-susceptibility-return|The actual compact return]] now establishes the quadratic covariance and susceptibility comparison for each fixed finite profile family. [[vacuum-hellinger-return-and-regional-conditional-projections|The conditional-projection theorem]] transfers the positive one-face semigroup defect to the actual compact kernels. Complete marked histories and growing source inventories remain additional obligations. Replacing the inherited law by an independent block misses its vacuum and memory.
