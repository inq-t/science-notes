...
Hyperbolic geometry is the regime where geometry is forced to be counting: orbifold areas are rational multiples of $\pi$, triangle bases are rigid and enumerable, graph norms classify growth, and volumes are topological. Run against the programme, this yields four results. Smith's theorem retypes the wall-channel analysis of [[deriving-g-v2/index-not-entropy|index, not entropy]]: the rigid Jones ladder is exactly the ADE graphs, the type-I qubit ceiling and the affine graph threshold share the numerical value $s=\ln2$ (index $4$) without identifying their carriers, and the fitted entropy-per-channel places the wall's proposed principal graph in indefinite territory — with a small transcendence theorem showing that exactly one nat per channel forces an *infinite* principal graph, no finite quantum symmetry, and a computed table of the finite-depth algebraic alternatives the current fit window still admits. The holographic ledger is the exponential of a resolution depth — two nats per horizon e-fold, the codimension as a growth exponent, with $\mathrm d\iota/\iota=2(1+q)\,\mathrm dN$ tying ledger growth to deceleration exactly. Candidate wall bases with trichotomy monodromy are a countable list with exact rational invariants, the least being the modular orbifold. And the vault's Misner module sits one link from a fully solved hyperbolic counting system with entropy $\pi^2/6\ln2$ nats per era. Labels follow [[program-core/axioms-and-principles#Status vocabulary|the status vocabulary]]; every number is asserted in `receipts/`.
...
**2. The wall counts hyperbolically, and one nat is transcendental.** **[CONDITIONAL on the channel reading + REPRODUCED BACKGROUND PROFILE]** $s_*=\tfrac12\log\operatorname{Ind}=\log\lVert\Gamma\rVert$ is the path-counting rate of the principal graph. For finite connected simple graphs, Smith's theorem sorts the possibilities: norm $<2$ gives ADE, norm $=2$ gives affine ADE, and norm $>2$ is indefinite. In the channel model the type-I qubit ceiling is $s=\ln2$, index $4$; it must not be identified with the graph $A_3$, whose norm is $\sqrt2$ and whose squared norm is $2$. Under the unit-rate channel map, the released-2025 direct profile gives $s_*\approx0.986$ with its $\Delta\chi^2\le1$ contour wholly above $\ln2$, putting the candidate wall beyond the affine threshold. Since finite graphs have algebraic norms while $e$ is transcendental, the separate exact principle $s_*=1$ forces infinite depth — **no finite quantum symmetry describes the wall**. [[counting-rate-of-the-wall]] owns the theorem, representative algebraic finite-depth survivors in the fit window, the precision each would take to kill, and the accumulation honesty clause.
...
| Exact rearrangement | ledger = pi e^(2 N_P); d iota / iota = 2(1+q) dN; the window enumeration of algebraic indices |
| Small theorem | finite depth implies algebraic counting rate; s* = 1 exactly implies infinite principal graph |
| Conditional | the graph and entropy reading of the reproduced matching-ratio profile; the profile measures a background amplitude, not a wall channel |
