# A Haar-Vertex Source for Joint Gauge Response

One graph source can return the gauge constraint, a positive state and the shared electric-type response together. Haar vertex frames provide exactly cost-free gauge motions, while retained heat factors on the edges provide nondegenerate observable variations. Their actual entropy Hessian fixes a mixed response on the complete invariant algebra, rather than requiring a second independently chosen Hamiltonian. The resulting weighted endpoint clock has an auxiliary lower bound uniform over finite graphs and respects ordinary edge subdivision. It is not a construction of the Yang--Mills vacuum or its physical translation generator, and the source clock does not in general descend literally to the endpoint readout.
...
[[heat-factor-response-and-the-compression-defect#Two sources, one complete path readout|the retained heat-factor construction]]
...
This differs from
[[two-sided-fisher-completion-and-the-neutral-carrier|joint output Fisher dualization]].
...
response in
[[oriented-context-gluing-and-mixed-response#Products and the shared response recover the whole algebra|the algebraic gluing construction]],
...
identity in
[[heat-factor-response-and-the-compression-defect#The output response has its own closed clock|the heat-factor source form]]
gives
\[
...
[[local-interaction-cocycle-and-global-source-response|Interacting local specifications]]
...
[[loop-correlations-and-the-source-response|Loop correlations]]
...
[[receipts/two_sided_fisher_receipt.py|The joint-source receipt]] checks
