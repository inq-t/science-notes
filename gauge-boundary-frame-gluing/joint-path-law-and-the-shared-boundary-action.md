# A Joint Path Law Forces a Shared Boundary Action

Two loop paths sharing one reference path cannot be transformed independently as though their separate laws were the whole state. Within deterministic finite-energy two-sided loop transformations, their joint heat-path law forces one common right boundary action. Violating that condition changes an observable quadratic variation and makes the joint laws mutually singular, although each transformed loop marginal is admissible separately. The compatible family is closed under changing the reference edge at a fixed base vertex. This selects a comparison structure from the joint law, rather than obtaining compatibility by defining one action to be the transport of another.
...
[[path-shift-fisher-geometry-before-gauge-projection|path-shift Fisher geometry]]
...
[[loop-correlations-and-the-source-response|tilted-source coercivity argument]].
...
[[source-action-transport-through-ordered-cuts|source-comparison programme]]
...
[[receipts/two_sided_fisher_receipt.py|The source receipt]] checks
