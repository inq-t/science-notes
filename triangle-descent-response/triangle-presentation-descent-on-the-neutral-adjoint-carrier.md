...
Then \(A^3=B_\theta^4=I\), and the representation is irreducible throughout the displayed interval. [[categorical-gauge-response/global-discreteness-kazhdan-rigidity-and-the-gap]] uses this family to show that the cusp displacement can nevertheless approach zero.
...
[[triangle-descent-response/receipts/triangle-neutral-adjoint-frame-receipt.py|The finite receipt]] checks the cyclic relations, expectation projections, frame spectrum, Hessian identity, and orientation spectrum; its [[triangle-descent-response/receipts/triangle-neutral-adjoint-frame-receipt-output.txt|stored output]] records representative angles.
