...
The exact gain is now visible. The cosmos can be modeled without imagining a container leaking into a larger spatial exterior: the apparent-horizon system already has a rigorous internal rate ledger. [[global-local-response-reconstruction/cosmological-reconvergence-contract|The cosmological reconvergence contract]] states the two realization maps now required. [[global-local-response-reconstruction/radiation-era-horizon-confinement-no-go|The radiation-era hierarchy]] also blocks the easiest wrong realization: in the recovered Einstein--FLRW branch, a Hubble fundamental mode is Planck-suppressed relative to a QCD thermal scale, so the horizon cannot be an ordinary glueball cavity. The open Copernican move is to construct one deeper whole-to-part law whose cosmological shadow is this ledger and whose local spectral shadow is a positive invariant mass floor. Until that common carrier and both maps exist, “cosmos as one geon” is a strong theorem-shaped hypothesis, not yet a solution of the mass-gap problem.
