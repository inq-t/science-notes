...
**The CST-B2 crossing epoch is adjacent but systematically early, not matching.** This member predicts $w_X<-1$ before its density maximum and $w_X>-1$ after it, with $w_X=-1$ at the maximum. The reconstructed sign pattern is compatible with that conditional member prediction; it does not confirm a microscopic response sector. The epoch is another matter. Predicted $z_c$ spans $0.24$ to $0.34$; solving each CPL pair for its own crossing gives $0.35$ to $0.50$. Those ranges do not overlap. Every combination crosses later than predicted, by $+0.014$ for Pantheon+ up to $+0.25$ for BAO+CMB — the same direction as the $w_0$ offsets, which is at least coherent, and not a match.
...
**Kills CST-B2 as the proposed realization of the reclassification.** A regular, minimally coupled, positive-kinetic scalar history — or a positive-definite sigma-model history — that reproduces the full CST-B2 crossing would directly defeat [[field-completion-no-gos|the member-specific no-go]]. Note the narrowness: exhibiting some other covariant response completion is not a falsifier but the thing the programme is trying to build. Also fatal to CST-B2 are a reconstructed member history that cannot satisfy $9(1+w_X)^2+6w_X'=$ any constant, subject to the power caveat above; more than one genuine density maximum or $w=-1$ crossing; or no phantom crossing. None of these observations alone falsifies the abstract CST response-family interface.
...
Sources for the observational values in layer three: DESI Collaboration, *DESI DR2 Results II: Measurements of Baryon Acoustic Oscillations and Cosmological Constraints*, [[library/desi-dr2-results-ii/inq|arXiv:2503.14738]], Table 5 and Eqs. (25)–(28); significances cross-checked against M. Cortês and A. R. Liddle, [[library/on-desi-dr2-exclusion-of-lambda-cdm/inq|arXiv:2504.15336]].
