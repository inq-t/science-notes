...
A postulate with a bounded dimensionless range and a stated relation to another structure is a better place to be stuck than a postulate with neither. It is also a place where being wrong is cheap to detect, because $\mathfrak R_c$ is background-fittable — which is why [[causal-scale-theory/observables|the test hierarchy]] puts a direct forward fit first, and warns in the same table that the closure atlas is an internal admissibility test rather than independent evidence, since flatness is used in the closure. [[causal-scale-theory/scale-capacity|The capacity note]] adds the matching caution: fitting an amplitude near one supports the effective branch and does not establish the microscopic identity.
