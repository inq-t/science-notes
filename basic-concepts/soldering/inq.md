...
Thus each [[basic-concepts/fibers/inq|fiber]] of $F(M)$ is a $\operatorname{GL}(V)$-[[basic-concepts/torsors/inq|torsor]]: it contains every frame but no preferred frame.
...
After pullback to $P$, this becomes an equivariant identification of $\pi^*T_{X/S}$ with the trivial bundle $\mathcal O_P\otimes V$. Local matrices for the identification must transform compatibly on overlaps; [[basic-concepts/gluing/inq|gluing]] constructs the bundle from those matrices, while [[basic-concepts/descent/inq|descent]] expresses why the equivariant local maps determine a global map.
