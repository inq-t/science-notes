...
For an $\mathcal O_X$-module $\mathcal F$ on a scheme, the distinction from a [[basic-concepts/fibers/inq|fiber]] is exact:
...
Not every sheaf is the section sheaf of an ordinary bundle, and a stalk of the section sheaf consists of germs of local sections rather than points of the bundle fiber. Frame choices form [[basic-concepts/torsors/inq|torsors]]; their transition functions describe how locally free sheaves become globally twisted.
...
The Zariski, étale, fppf, and fpqc topologies declare different morphisms to be local covers. The same assignment may be a sheaf for one topology and fail for a finer one because the finer topology demands effective gluing across more presentations. This is why claims that an object exists “locally” are incomplete until the topology is named; [[basic-concepts/descent/inq|descent]] develops that dependence in detail.
...
| [[basic-concepts/stacks/inq|Stack]] | category- or [[basic-concepts/groupoids/inq|groupoid]]-valued descent | objects and their automorphisms glued up to coherent isomorphism |
