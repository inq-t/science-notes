...
This distinction is a small instance of a general lesson developed in [[basic-concepts/fibers/entry|fibers]] and [[basic-concepts/soldering/entry|soldering]]: data living over nearby points cannot be compared until a comparison law has been specified.
...
This is one reason moduli problems are naturally expressed using [[basic-concepts/groupoids/entry|groupoids]] and [[basic-concepts/stacks/entry|stacks]]. Automorphisms are not noise to be deleted from the presentation: they determine the degree-$-1$ part of the deformation complex and hence the correct domain on which a second variation can be nondegenerate.
...
is an isomorphism. This is the algebraic analogue of an isolated Morse critical point. In a family $X\to S$, one may similarly study the relative differential and the vertical Hessian along $T_{X/S}$; this measures degeneracy inside each [[basic-concepts/fibers/entry|fiber]], not variation along the base.
...
For a smooth scheme, sheaves of principal parts and jet bundles package these local Taylor data. Their filtrations have graded pieces built from symmetric powers of $\Omega^1_{X/S}$, but a filtration need not split canonically. Local arrays of second derivatives therefore do not [[basic-concepts/gluing/entry|glue]] as a global tensor merely because their entries agree syntactically. One needs a connection, a critical-locus restriction, or some other invariant construction. [[basic-concepts/sheafs/entry|Sheaves]] are what make the locality of the coefficients precise; descent of the resulting object is a separate question.
...
The linear term vanishes because relative entropy is minimized on the diagonal. Faithfulness and regularity matter: at the boundary of state space, supports can change and the quadratic expansion can diverge or cease to exist. In continuum QFT, local algebras are generally not described by trace-class density matrices, so one needs an operator-algebraic relative entropy and a controlled class of perturbations rather than this finite-dimensional formula by assertion.
...
## Use in Causal Scale Dynamics and CWST

The project uses Hessians in several logically distinct places. Keeping their statuses separate turns a suggestive dictionary into a sequence of explicit proof obligations.

### The reduced binary family

In [[causal-scale-theory/binary-geometry|balanced binary information geometry]], the stipulated balanced two-outcome family has

$$
\omega_\theta
=\frac{e^{\theta Q}}{2\cosh\theta},
\qquad
\Psi(\theta)=\log(2\cosh\theta),
$$
...
Within that commuting binary reduction, this is exactly and simultaneously the log-partition Hessian, ordinary variance of $Q$, Fisher metric coefficient, and BKM metric coefficient. It does **not** follow that a full causal-wall QFT is binary, that its physical horizontal scale tangent is generated by $Q$, or that the affine coordinate $\theta$ equals a Weyl e-fold. Those are the state-construction and [[causal-scale-theory/scale-soldering|scale-soldering]] obligations.

### The free-energy expansion

[[causal-scale-theory/relative-entropy-hessian|The relative-entropy Hessian]] supplies the local expansion used by [[causal-scale-theory/free-energy-source|the modular free-energy source law]],
...
Any claim that the programme recovers a perturbative QFT sector needs a covariant quadratic action or exact 1PI functional whose Hessian has a stated field content, gauge complex, domain, causal prescription, and stable physical spectrum. One must then establish locality or the intended controlled nonlocality, hyperbolicity or Euclidean ellipticity as appropriate, absence of ghosts, and the relation of its inverse to observable correlators. The internal Witten factorization and the positive binary BKM form are useful quadratic structures, but neither is by itself the spacetime perturbation Hessian required by [[compatible-with-existing-physics/local-physics-interface|the local-physics interface]].

The durable diagnostic is therefore simple:
