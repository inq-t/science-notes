...
Gluing concerns neighborhoods, not isolated points. The individual [[basic-concepts/fibers/entry|fibers]] $E_x$ may describe what lies over each $x\in X$, but a bare list of fibers contains no information about which nearby elements vary together, which local sections exist, or how transport around a loop acts. The overlap maps carry that missing relational structure.
...
with $e_0=t e_1$ on $t\ne0$. The function $t$ is not an incidental coordinate formula: it is the rule by which two trivial local line bundles become one globally nontrivial line bundle. Principal bundles, gauge fields, and [[basic-concepts/torsors/entry|torsors]] use the same architecture with a group-valued transition cocycle.
...
| [[basic-concepts/descent/entry|Descent]] | Can an object pulled to a cover be recognized as coming from the base? |
| [[basic-concepts/soldering/entry|Soldering]] | How are objects of different geometric or physical kinds identified or calibrated? |
