...
The orbit set \(X/G\) forgets every stabilizer. The action groupoid retains them, including the fact that an object can be equivalent to itself in several nontrivial ways. This is the prototype of a quotient groupoid and, after imposing descent, of the quotient [[basic-concepts/stacks/entry|stack]] \([X/G]\).
...
from \(x\) to \(y\): equivariant bijections between the two \(G\)-[[basic-concepts/torsors/entry|torsor]] fibers. Equivalently, its arrow bundle is
...
Pullback along \(T'\to T\) gives a functor between groupoids. When these groupoids satisfy descent, they form a [[basic-concepts/stacks/entry|stack]]. Replacing each \(\mathcal M(T)\) by its set of isomorphism classes too early can destroy the gluing information, because locally chosen isomorphisms can differ by automorphisms on overlaps.
