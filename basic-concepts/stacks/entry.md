...
A stack is a local-to-global object that remembers symmetry. Over every test space it assigns a [[basic-concepts/groupoids/entry|groupoid]] of families and their isomorphisms; pullback changes the test space, and the stack condition says that compatible local families and compatible arrows satisfy effective [[basic-concepts/descent/entry|descent]]. A stack is therefore a categorified [[basic-concepts/sheafs/entry|sheaf]]: equality of local sections is replaced by coherent isomorphism of local objects. Algebraic stacks add representability conditions that make this symmetry-sensitive moduli object accessible to algebraic geometry.
...
This use of *fiber* is categorical. It is related in spirit to [[basic-concepts/fibers/entry|scheme-theoretic fibers]] but is not the same construction as \(X\times_S\operatorname{Spec}\kappa(s)\).
...
A morphism \(T\to BG\) therefore *is* a family of \(G\)-torsors over \(T\). The [[basic-concepts/torsors/entry|torsor]] need not be globally trivial; allowing all such torsors is exactly what makes the construction satisfy descent.
...
The action [[basic-concepts/groupoids/entry|groupoid]]
...
- classification of [[basic-concepts/torsors/entry|torsors]] and forms by descent.
